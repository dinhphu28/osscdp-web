...
| Events and audit **keyset pagination** (`limit`/`cursor` → `next_cursor`)                            | Generated hook may not model cursor state | Wrap in `useInfiniteQuery`; drive Data Grid server mode.                                                       |
