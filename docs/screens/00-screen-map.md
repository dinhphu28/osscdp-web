...
| 2   | App shell                 | (layout at `/t/:tenantId`)                                                               | `GET /admin/v1/whoami` (role auto-detect), `GET /admin/v1/tenants` (list, super-admin switcher)                                                                                                               | none (token present)                                                                                  | (chrome, not a nav item) |
| 3   | Dashboard                 | `/t/:tenantId/dashboard`                                                                 | `GET /healthz`, `GET /readyz`, `GET /admin/v1/tenants/{tenantID}/stats` (JSON counts), `GET /metrics` (Prometheus text), `GET /admin/v1/tenants/{tenantID}/dlq?status=open`                                    | read set (`source:read`…`dlq:read`)                                                                   | Overview                 |
| 4   | Sources                   | `/t/:tenantId/sources`                                                                   | `GET /admin/v1/tenants/{tenantID}/sources` (list), `POST /admin/v1/tenants/{tenantID}/sources`, `POST /admin/v1/tenants/{tenantID}/sources/{sourceID}/rotate-key`                                              | view: `source:read`; create/rotate/disable: `source:write`                                            | Ingest                   |
...
| 11  | Audit log                 | `/t/:tenantId/audit`                                                                     | `GET .../audit` — keyset-paginated (`limit`/`cursor` → `next_cursor`); metadata-only entries (`created_at, actor_type, action, resource_type, resource_id`), no before/after JSON                              | `audit:read`                                                                                          | Operations               |

Notes:

- **Discard shares `dlq:retry`** (there is no separate `dlq:discard` permission).
- `DELETE .../segments/{id}` is **not in `openapi.yaml`** (code-only) — see [backend gaps](../10-backend-gaps-and-caveats.md) item 5. (`GET .../segments` list now exists.)
- `/metrics` is **Prometheus text, not JSON** — the dashboard uses `GET .../stats` for real counts and links/embeds Grafana rather than parsing `/metrics` (gap item 9).
...
/t/:tenantId            → AppLayout (nav rail + top bar + tenant switcher + <Outlet/>)
  ...
  ├─ /dlq
  ├─ /audit            (live — keyset audit table, metadata-only)
...
| 8     | Audit          | `/t/:tenantId/audit`          | Operations     | `audit:read`                           |
...
Roles: `SUPER_ADMIN`, `TENANT_ADMIN`, `MARKETER`, `ANALYST`, `OPERATOR`, `VIEWER`. Permissions per role are the canonical role→permission table (reproduced in the auth doc and [Data model & types](../07-data-model-and-types.md)). The console auto-detects the token's role from `GET /admin/v1/whoami` (`{role, tenant_id, is_super_admin}`) and computes permissions client-side; the manual role picker at `/connect` is only a fallback if `whoami` `404`s.
...
| ------------------------------------------------------------------------------------------- | ----------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Audit log — before/after JSON diff                                                          | `/t/:tenantId/audit`    | **Not available (intentional)**              | `GET .../audit` is **live** (keyset-paginated), but returns **metadata only** (`created_at, actor_type, action, resource_type, resource_id`). The `before`/`after` diff is omitted on purpose (PII) — do not render a diff column.               | [gap #2](../10-backend-gaps-and-caveats.md), [audit doc](10-audit-log.md) |
...
- [ ] The Audit screen renders a live keyset-paginated table from `GET .../audit` (metadata-only columns; no before/after diff).
