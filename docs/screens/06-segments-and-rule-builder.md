...
export interface RuleNode { operator: LogicalOp; conditions: Rule[]; }   // 'and' | 'or' | 'not'
...
```ts
const ruleLeaf: z.ZodType<RuleLeaf> = z.object({
  field: z.string().min(1),
  op: z.enum(['eq','neq','gt','gte','lt','lte','contains','not_contains','in','not_in','exists','not_exists']),
  value: z.unknown().optional(),
}).refine(l =>
  (l.op === 'exists' || l.op === 'not_exists') ? l.value === undefined :
  (l.op === 'in'     || l.op === 'not_in')     ? Array.isArray(l.value) :
...
export interface RuleLeaf { field: string; op: RuleOp; value?: unknown | unknown[]; }
export interface BehaviorLeaf { behavior: { /* see above */ }; }

export interface Segment {
  id: string; name: string; description?: string;
  status: string; current_version_id?: string;
}
export interface SegmentVersion { version: number; rule_json: Rule; status: string; }
export interface SegmentMembership {
  customer_profile_id: string; status: 'member' | string;
  entered_at?: string; exited_at?: string | null;
  last_evaluated_at?: string; version?: number; transition_seq?: number;
}
```
